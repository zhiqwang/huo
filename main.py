...
from utils.art import scan, art

parser = argparse.ArgumentParser(description='Algebraic reconstruction technique')

# export directory, training and val datasets, test datasets
parser.add_argument('--img-pixels', default=512, type=int, metavar='NX',
                    help='sample pixels of image')
parser.add_argument('--img-len', default=144, type=float, metavar='SX',
                    help='length of the image, also the diameter of the FOV (unit: mm)')
parser.add_argument('--detr-num', default=500, type=int, metavar='NU',
                    help='number of detector unit')
parser.add_argument('--detr-len', default=180, type=float, metavar='SU',
                    help='length of the detector panel (unit: mm)')
parser.add_argument('--lat-sampling', default=2, type=int,
                    help='sample grid of X-ray in longitude direction')
parser.add_argument('--sdd', default=1200, type=float,
                    help='distance of the X-ray source to detector panel (unit: mm)')
parser.add_argument('--sod', default=981, type=float,
                    help='distance of the X-ray source to object axis (unit: mm)')
parser.add_argument('--off-u', default=0, type=float,
                    help='detector rotation shift length (unit: mm)')
parser.add_argument('--rotate-step', default=1.0, type=float,
                    help='longitude step (deg) of ray')
parser.add_argument('--direction', default=-1, type=float,
                    help='gantry rotating direction (clockwise / counter clockwise')
parser.add_argument('--max-epoch', default=10, type=int, metavar='N',
